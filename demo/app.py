...
class MultiScaleSpectroBlock(nn.Module):
    ...
    def forward(self, x):
        out = torch.cat(
            [self.conv_std(x), self.conv_tmp(x), self.conv_frq(x)], dim=1
        )
...
demo_resnet.load_state_dict(
    torch.load('best_resnet50.pth', map_location=device)
)
demo_resnet.eval()
...
def predict_and_visualize(audio_path):
    ...
    buf = io.BytesIO()
    fig.savefig(
        buf, format='png', bbox_inches='tight', facecolor='#0f172a'
    )  # Match background
    buf.seek(0)
    ...
    for i in range(10):  # 10 chunks * 3 seconds = 30 seconds total
        ...
        if mel_chunk.shape[2] < frames_per_chunk:
            mel_chunk = F.pad(
                mel_chunk, (0, frames_per_chunk - mel_chunk.shape[2])
            )
    ...
    confidences = {
        genres[i]: float(final_prob[i]) for i in range(len(genres))
    }
...
music_theme = gr.themes.Default(
    ...
    font=[gr.themes.GoogleFont("Outfit"), "sans-serif"]  # Modern font
).set(
    body_background_fill="#0f172a",          # Very dark slate background
    body_background_fill_dark="#0f172a",
    block_background_fill="#1e293b",         # Slightly lighter slate panels
...
with gr.Blocks() as interface:
    gr.Markdown(
        ...
        "Deep Audio Genre Classifier</h1>"
    )
    gr.Markdown(
        "<p style='text-align: center;'>Upload an MP3/WAV! The Soft-Voting "
        "Ensemble will slice the first 30 seconds into 3-second chunks and "
        "analyze the acoustic textures.</p>"
    )
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            image_output = gr.Image(
                label="Mel Spectrogram (30s Window)", type="pil"
            )
            label_output = gr.Label(
                num_top_classes=4, label="Ensemble Prediction"
            )
